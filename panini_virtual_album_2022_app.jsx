...
import { db, doc, getDoc, setDoc, onSnapshot, arrayUnion } from './firebase_2022';
  ...
  useEffect(() => {
    ...
    const fetchProgress = async () => {
      ...
      const entries = await Promise.all(
        PROYECTOS.filter(proyecto => proyecto.id !== albumConfig.id).map(async (proyecto) => {
          try {
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
        ...
        {currentView === 'home' && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
            ...
            <button onClick={() => setCurrentView('repetidas')} className={`rounded-3xl p-6 text-left font-black text-lg cursor-pointer transition-all hover:scale-105 active:scale-95 shadow-lg ${darkMode ? 'bg-[#1e1e30] text-white' : 'bg-white'}`}>
              🔁<br/>
      ...
      {currentView === 'repetidas' && (() => {
        ...
        {currentView === 'faltan' && (
          <div className={`rounded-3xl p-6 sm:p-8 shadow-xl max-w-2xl mx-auto transition-colors duration-300 ${darkMode ? 'bg-[#1e1e30] text-white' : 'bg-white'}`}>
            ...
            {faltantesGrouped.length === 0 ? (
              ...
              <div className="space-y-4 max-h-[55vh] overflow-y-auto pr-1 mb-4">
                {faltantesGrouped.map(({ team, info, codes }) => (
                  <div key={team} className={`rounded-2xl p-4 ${darkMode ? 'bg-[#2a2a4a]' : 'bg-slate-50'}`}>
                    <div className="flex items-center gap-2 mb-3">
                      <span className="text-2xl leading-none">{info?.flag || '🏳️'}</span>
                      <div>
                        <div className="font-black uppercase text-sm">{info?.name || team}</div>
            ...
            <div className={`mt-4 pt-4 border-t flex flex-wrap gap-3 ${darkMode ? 'border-[#2a2a4a]' : 'border-slate-200'}`}>
              <button
            ...
            {showExportTextFaltan && (() => {
              ...
              const text = `Figuritas que le faltan a ${ALBUM_OWNER} - Mundial Qatar 2022\n\n${lines.join('\n')}`;
              return (
                <div className="fixed inset-0 z-[70] bg-black/60 flex items-center justify-center p-4">
                  <div className={`rounded-3xl p-6 shadow-2xl w-full max-w-lg ${darkMode ? 'bg-[#1e1e30] text-white' : 'bg-white'}`}>
                    ...
                    <div className="flex gap-3 mt-4">
                      ...
                      <button
                        ...
                        className={`flex-1 px-4 py-3 rounded-2xl font-black ${darkMode ? 'bg-[#2a2a4a] text-white' : 'bg-slate-200 text-slate-800'}`}
                      >
                        CERRAR
                      </button>
                    </div>
...
function ProgressHistoryModal({ history, darkMode, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4">
      <div className={`rounded-3xl p-6 sm:p-8 shadow-2xl w-full max-w-2xl transition-colors duration-300 ${darkMode ? 'bg-[#1e1e30] text-white' : 'bg-white'}`}>
        ...
        {rows.length === 0 ? (
          ...
          <div className="max-h-[60vh] overflow-auto rounded-2xl border border-slate-300/30">
            <table className="w-full text-sm">
              <thead className={`sticky top-0 ${darkMode ? 'bg-[#1a1a2e]' : 'bg-slate-100'}`}>
              ...
              <tbody>
                {rows.map((entry) => (
                  <tr key={entry.id ?? entry.dateLabel} className={`border-t ${darkMode ? 'border-[#2a2a4a]' : 'border-slate-200'}`}>
